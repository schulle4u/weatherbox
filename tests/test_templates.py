...
def test_render_valid_template(location, weather, now):
    context = build_context(location, now.replace(hour=14, minute=0), weather)
...
def test_unknown_variable_fails(location, weather, now):
    context = build_context(location, now, weather)
    with pytest.raises(TemplateRenderError, match="Unbekannte"):
        render_template("{not_a_field}", context)


def test_missing_weather_value_fails(location, weather, now):
    context = build_context(location, now, weather)
...
def test_unused_missing_value_is_allowed(location, weather, now):
    context = build_context(location, now, weather)
